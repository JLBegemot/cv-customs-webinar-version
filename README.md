...
Упрощённая версия сервиса cv-customs: **авторизация, загрузка файлов резюме,
настройки пользователя**. Файлы хранятся как есть (метаданные в Postgres +
блоб в S3/MinIO) — без парсинга, LLM, фоновых задач, Redis и rate-limit'ов.

> **Только локальный запуск.** В сервисе нет rate limiting и подтверждения
> email — выставлять его наружу нельзя. Это осознанное ограничение
> упрощённой версии.
